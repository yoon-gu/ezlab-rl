...
def plot_curves(problem_name,
    xy_list: List[Tuple[np.ndarray, np.ndarray]], x_axis: str, title: str, figsize: Tuple[int, int] = (8, 8)
) -> None:
    """
    plot the curves
    ...
    plt.tight_layout()
    plt.savefig("Figures/Reward{}".format(problem_name))
    plt.savefig("Figures/Reward{}.svg".format(problem_name))


def plot_results(problem_name,
    dirs: List[str], num_timesteps: Optional[int], x_axis: str, task_name: str, figsize: Tuple[int, int] = (5, 3)
    #8, 2
) -> None:
    """
    ...
    data_frames = []
    for folder in dirs:
        if isinstance(folder, pd.DataFrame):
            data_frame = folder 
        else:
    ...
    plot_curves(problem_name, xy_list, x_axis, task_name, figsize)