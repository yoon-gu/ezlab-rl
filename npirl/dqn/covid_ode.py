...
def covid(y, t, dt, t_idx, mV1, mV2, e1, e2, kappa, alpha, gamma, vprevf1, vprevf2, fatality_rate, 
    ...
    for i in range(int(1/dt)):
        ...
        S_next = - lambdaS - mv1 
        E_next = lambdaS - kappa * E
        I_next = kappa * E - alpha * I
        H_next = alpha * (I + IV1 + IV2) - gamma * H
        R_next = gamma * H
        V1_next = mv1 - lambdaV1 - mv2
        V2_next = mv2 - lambdaV2
        EV1_next = lambdaV1 - kappa * EV1
        EV2_next = lambdaV2 - kappa * EV2
        IV1_next = kappa * EV1 - alpha * IV1
        IV2_next = kappa * EV2 - alpha * IV2
        F = alpha * (I + (1 - vprevf1) * IV1 + (1 - vprevf2) * IV2) * fatality_rate
        SI = alpha * (I + (1 - vprevs1)* IV1 + (1 - vprevs2) * IV2) * severe_illness_rate
        new_inf = alpha * (I_next+ IV1_next + IV2_next)
...
class covidEnvironment:
    ...
    def step(self, action, t_idx):
        ...
        sol = odeint(covid, 
                    y0, 
                    np.linspace(0,1,101), 
                    args=(self.dt, t_idx, self.mV1, self.mV2,
