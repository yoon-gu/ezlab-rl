...
@hydra.main(version_base=None, config_path="conf", config_name="covid7")
def main(conf : DictConfig) -> None:
    ...
    def covid7(y, dt, t_idx, mV1, mV2, e1, e2, kappa, alpha, gamma, vprevf1, vprevf2, fatality_rate, 
        ...
        neg_flag_S_hist = neg_flag_S_hist.reshape(-1)
        neg_flag_V1_hist = neg_flag_V1_hist.reshape(-1)
        neg_flag_S = np.zeros(9, dtype = np.bool_)
        neg_flag_V1 = np.zeros(9, dtype = np.bool_)
    ...
    agent = Agent(state_size=126, action_size=4, seed=0, scale=conf.scale)
    ## Parameters
    ...
    plt.clf()
    #plt.plot(range(26), S, '.-',label='S', linestyle='dashed')
    ...
    plt.plot(sd_RL, '.-r', label = 'control')
