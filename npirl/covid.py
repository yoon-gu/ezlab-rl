...
M = loadmat('/Users/boyeonkim/research/npirl/sir/params.mat')
MV = loadmat('/Users/boyeonkim/research/npirl/sir/mv1.mat')
...
def covid(y, dt, t_idx, mV1, mV2, e1, e2, 
          kappa, alpha, gamma, vprevf, fatality_rate, vprevs, 
    ...
    for i in range(int(1/dt)):
        ...
        F_next = F + (alpha * (I + (1 - vprevf[0]) * IV1 + (1 - vprevf[1]) * IV2) * fatality_rate) * dt
        SI_next = SI + (alpha * (I + (1 - vprevs[0])* IV1 + (1 - vprevs[1]) * IV2) * severe_illness_rate) * dt
...
class covidEnvironment:
    def __init__(self):
        ...
        self.vprevs = M['vprevs'][0]
        self.vprevf = M['vprevf'][0]
    ...
    def step(self, action, t_idx):
        ...
        sol = covid(y0, self.dt, t_idx, self.mV1, self.mV2,
                    ...
                    self.vprevf, self.fatality_rate, self.vprevs, self.severe_illness_rate, 
