function result = solve_covid19(theta,M,index)
...
if sc_flag == 0
    ...
    while i <= length(M.time_stamp)-2
        ...
        for j = 1 : length(1:1/M.dt)
            ...
            new_inf(:,i,j+1) = new_inf(:,i,j) + M.dt * M.alpha *((I(:,i,j) + IV1(:,i,j) + IV2(:,i,j)) + (I(:,i,j+1) + IV1(:,i,j+1) + IV2(:,i,j+1))) / 2;
            F(:,i,j+1) = F(:,i,j) + M.dt * (M.alpha * (I(:,i,j) + (1-M.v_prevent_f(1)) * IV1(:,i,j) + (1-M.v_prevent_f(2)) * IV2(:,i,j)) .* M.fatality_rate');
            SI(:,i,j+1) = SI(:,i,j) + M.dt * (M.alpha * (I(:,i,j) + (1-M.v_prevent_s(1)) * IV1(:,i,j) + (1-M.v_prevent_s(2)) * IV2(:,i,j)) .* M.severe_illness_rate');
    ...
    % school effect is back to original
elseif sc_flag == 1
    i = 0;
    % Loop for time_stamp
    while i <= length(M.time_stamp)-2
        ...
        WAIFW = beta * M.contact * delta_eff(i) * M.sd(i);
        if i >= M.sc_index
            WAIFW(2,2) = (M.sc_rate / M.sd(i)) * WAIFW(2,2);
        end
        % Loop for tspan
        for j = 1 : length(1:1/M.dt)
            lambdaS = WAIFW * (I(:,i,j) + IV1(:,i,j) + IV2(:,i,j)) .* S(:,i,j);
